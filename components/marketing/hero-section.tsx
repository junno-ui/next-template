...
/* -------------------------------------------------------------------------- */
/*  Trusted-By Logos (text-based to match reference)                          */
/* -------------------------------------------------------------------------- */
const trustedLogos = [
  ...
  return (
    <section id="home" className="relative overflow-hidden">
      ...
      >
        ...
        <div className="absolute top-[-10%] left-1/2 h-[700px] w-[1000px] -translate-x-1/2 rounded-full bg-primary/[0.06] blur-[120px]" />
        {/* Bottom-right subtle accent */}
        <div className="absolute right-[-10%] bottom-[-5%] h-[500px] w-[700px] rounded-full bg-primary/[0.03] blur-[100px]" />
        {/* Left dim accent */}
        <div className="absolute top-[40%] left-[-5%] h-[400px] w-[400px] rounded-full bg-chart-2/[0.04] blur-[100px]" />
      </div>

      {/* ── Hero Content ────────────────────────────────────────────────── */}
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center px-6 pt-28 pb-12">
        {/* Welcome badge */}
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/[0.06] px-4 py-1.5 text-xs font-medium tracking-wide text-primary backdrop-blur-sm sm:text-sm">
          <Icon icon="mdi:star-four-points" className="size-3.5" />
          Welcome to PixarMarketing
        </div>

        {/* Heading */}
        <h1 className="mx-auto max-w-4xl text-center text-4xl font-bold leading-[1.08] tracking-tight text-foreground sm:text-5xl md:text-6xl lg:text-[3.75rem]">
          Optimize Your
          <br />
        ...
        <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row">
          <a
            ...
            className={cn(
              ...
              "rounded-full border-border/40 px-6 text-sm hover:bg-muted/50"
            )}
          >
            Explore Features
          </a>
          <a
            ...
            className={cn(
              ...
              "rounded-full px-6 text-sm shadow-lg shadow-primary/25 transition-shadow hover:shadow-xl hover:shadow-primary/35"
            )}
          >
            Try for Free Now
        ...
        <div className="relative mx-auto mt-16 w-full max-w-5xl">
          ...
          <div
            className="pointer-events-none absolute -inset-8 -z-10 rounded-3xl bg-gradient-to-b from-primary/[0.08] via-transparent to-transparent blur-3xl"
            aria-hidden="true"
          />

          <div className="overflow-hidden rounded-xl border border-border/30 bg-card/80 shadow-2xl shadow-black/20 backdrop-blur-sm">
            <div className="flex">
              {/* ── Sidebar ──────────────────────────────────────────────── */}
                ...
                <nav className="flex flex-col gap-0.5 p-2">
                  {sidebarItems.map((item) => (
                    <div
                      key={item.label}
                      className={cn(
                        "flex items-center gap-2.5 rounded-lg px-3 py-2 text-[11px] font-medium transition-colors",
                        item.active
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground/60">
                      Here is a friendly report and perfect data here
                    </p>
                  </div>
                  ...
                  {[
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Icon icon={stat.icon} className="size-3 text-muted-foreground/30" />
                      </div>
                      ...
                      <p className={cn(
                        ...
                        stat.change.startsWith("▲") ? "text-primary" : "text-red-400"
                      )}>
                        {stat.change}
                      </p>
                    </div>
                  ...
                  <div className="rounded-lg border border-border/15 bg-muted/20 p-3 sm:col-span-3">
                    ...
                    <div className="flex h-28 items-end gap-1.5">
                      {[
                        ...
                        <div
                          key={i}
                          className="flex flex-1 items-end gap-[1px]"
                        >
                          <div
                            className="flex-1 rounded-t-sm bg-primary/30 transition-all hover:bg-primary/50"
                            style={{ height: `${bar.h1}%` }}
                          />
                          <div
                            className="flex-1 rounded-t-sm bg-chart-2/25 transition-all hover:bg-chart-2/40"
                            style={{ height: `${bar.h2}%` }}
                          />
                        </div>
                      ))}
        ...
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
            {trustedLogos.map((logo) => (
              <div
                key={logo.name}
                className="flex items-center gap-2 text-muted-foreground/30 transition-colors hover:text-muted-foreground/60"
              >
                <Icon icon={logo.icon} className="size-5" />
